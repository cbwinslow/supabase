...
function PartnerManagedResource({ partner, resource, cta }: PartnerManagedResourceProps) {
  ...
  const vercelQuery = useVercelRedirectQuery(
    ...
    {
      enabled: ctaEnabled && partner === MANAGED_BY.VERCEL_MARKETPLACE,
    }
  )
  ...
  const awsQuery = useAwsRedirectQuery(
    ...
    {
      enabled: ctaEnabled && partner === MANAGED_BY.AWS_MARKETPLACE,
    }
  )

  if (partner === MANAGED_BY.SUPABASE) return null

  const { data, isLoading, isError } =
    partner === MANAGED_BY.VERCEL_MARKETPLACE ? vercelQuery : awsQuery
